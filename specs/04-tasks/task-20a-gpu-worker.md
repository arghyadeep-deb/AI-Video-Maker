...
- **Hardware reality check (2026-07-10, first real `nvidia-smi` read)**: the card is a **5070 Ti *Laptop* GPU with 12 GB VRAM** (12227 MiB, driver 592.01), not the 16 GB desktop part the planning docs assumed. Consequences: SadTalker/MuseTalk/VoxCPM all still fit; scene_gen (~10-14 GB class models) relies on the already-enabled `enable_model_cpu_offload()`; **judgment gate #1's LongCat-quantized-into-16GB question is effectively answered — 13.6B does not fit 12 GB**, so Mode A HD realistically stays SadTalker (owner can still confirm with their eyes at the gate). Design doc corrected; venv layout also fixed during install (in-process engines share the agent venv; only SadTalker/MuseTalk get a separate pins venv via `engines_python`).
