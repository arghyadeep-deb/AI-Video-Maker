...
## R13 — Free `trycloudflare.com` quick tunnels have no uptime guarantee — MEDIUM, **triggered 2026-07-12**
The PC-hosted deploy (task-20's fallback for the Oracle card-decline) uses `cloudflared tunnel --url` — a free, unauthenticated "quick tunnel" with a randomly-assigned hostname and no official uptime SLA. One died silently after ~20h with no auto-restart (the one-shot launcher script had no supervision loop), leaving the live site down ("Failed to load your library") until caught and fixed by hand: fresh tunnel, new `NEXT_PUBLIC_API_BASE_URL` on Vercel, frontend redeploy. Every restart mints a new hostname, so recovery always needs that same manual Vercel update - there is no way to pin a stable URL without a real (paid-domain-backed) Cloudflare Tunnel. **Mitigation (not yet done):** switch `deploy/tunnel/run_backend_tunnel.cmd` to the self-restarting loop variant (`run_tunnel.cmd` already exists for this) so a dropped connection recovers automatically when possible; add an uptime monitor (e.g. a scheduled health-check hitting `/api/meta/health` through the public URL) so a genuine hostname rotation is caught within minutes instead of whenever someone happens to check. Oracle migration (R10) would retire this risk entirely.
